            ...
            <tbody className="divide-y divide-gray-200">
              {standings.map((player) => (
                <tr key={player.player} className={getRowColorClass(player)}>
                  <td className={`px-4 py-3 text-left font-bold sticky left-0 z-10 shadow-sm border-r ${getStickyBgClass(player)}`}>
                    <div className="flex items-center gap-1">
                      {player.rank === 1 && <Trophy className="w-4 h-4 text-yellow-600" />}
                      {player.rank}
                    </div>
                  </td>
                  <td className={`px-4 py-3 text-left font-medium sticky left-12 z-10 shadow-sm border-r ${getStickyBgClass(player)}`}>
                    <div className="flex items-center gap-1">
                      {player.player}
                    </div>
                  </td>
      ...
      {isMobile && (
        <div>
          {standings.map((player) => {
            ...
            return (
              <div key={player.player} className={`border-b last:border-b-0 ${getCardColorClass(player)}`}>
                <div
