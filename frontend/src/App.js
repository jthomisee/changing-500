              ...
              <div key={index} className="grid grid-cols-1 lg:grid-cols-7 gap-2 mb-3 p-3 bg-gray-50 rounded">
                <div>
                  ...
                  <input
                    type="text"
                    value={result.player}
                    onChange={(e) => updatePlayerResult(index, 'player', e.target.value)}
              ...
              .map(game => (
                <div key={game.id} className="border-b pb-4 mb-4 last:border-b-0">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => isAdmin && startEditingGame(game)}
                        ...
                        disabled={!isAdmin}
                        title={isAdmin ? "Edit game" : "Admin login required"}
                      >
                        <Edit className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => isAdmin && deleteGame(game.id)}
                        className={`p-2 rounded-lg ${
                          isAdmin 
                        ...
                        disabled={!isAdmin}
                        title={isAdmin ? "Delete game" : "Admin login required"}
                      >
